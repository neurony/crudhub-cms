<template>
    <PageHeader title="Edit page" :subtitle="`Last updated on ${lastUpdatedDate}`">
    ...
    data() {
        return {
            form: useForm({
                ...
                identifier: this.item.data?.identifier ?? null,
                slug: this.item.data?.slug ?? null,
                active: this.item.data?.active ?? 0,
                meta_data: this.item.data?.meta_data ?? {},
                meta_tags: this.item.data?.meta_tags ?? {},
                images: this.item.data?.images ?? [],
            }),
        }
    },
    methods: {
        updateRecord(saveStay = false) {
            ...
            this.form.put(route('admin.pages.update', this.item.data.id));
        },
    },
    computed: {
        lastUpdatedDate() {
