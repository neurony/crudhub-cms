<template>
    <div class="w-full xl:w-1/4 xl:mr-8 mb-8 xl:mb-0 py-4 px-6 bg-white overflow-hidden rounded-md shadow ring-1 ring-gray-600/5 ring-opacity-5">
        ...
        >
            <template #default="{ node, stat }">
                ...
                <span
                    ...
                    :class="[node.name === 'Menus' ? 'font-semibold' : 'text-gray-600']"
        ...
        <div class="bg-indigo-50 px-4 py-2.5 rounded-md col-span-6 mt-6">
            ...
            <div class="text-xs text-indigo-900 mb-4">
                Creating a new menu after clicking on a tree node, will create a child of that menu you clicked on from the tree.
            </div>
...
export default {
    ...
    methods: {
        fixTree() {
            router.post(route('admin.menus.tree.fix'), {}, {
                preserveState: true,
                preserveScroll: true,
            });
        },
        rebuildTree() {
            ...
            form.post(route('admin.menus.tree.rebuild'), {
                preserveState: true,
                preserveScroll: true,
            });
        },
        loadTreeLeaves(stat) {
            ...
            router.get(
                route('admin.menus.index', { location: this.location }),
                { ...this.query, parent: stat.data.id },
